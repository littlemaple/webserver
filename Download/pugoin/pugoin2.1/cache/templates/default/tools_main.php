...
<?php if($member['qq']) { ?>
			<div>
				<BR />你当前帐户已经与QQ<?php echo $member['qq']; ?>绑定了，现在可以：<BR />
				1、通过你的QQ发微博内容到QQ机器人<?php echo $this->Config['imrobot']; ?>，格式是“<span style='color:red'>新微博：你的微博内容</span>”；<BR>
				2、有人回复你的微博、或@提到你，你会接收到QQ机器人<?php echo $this->Config['imrobot']; ?>通知。<BR>
				3、通过QQ签名发微博：修改QQ签名后，机器人会自动获取签名内容并更新微博。<BR>
				4、如需取消QQ与帐户绑定，只需你的QQ号码<?php echo $member['qq']; ?>发给QQ机器人即可。<BR>
			</div>
<?php } else { ?><div><BR />
				你当前帐户还未绑定QQ，请按如下步骤设置绑定：<BR />
				1、请用您的QQ加QQ机器人<?php echo $this->Config['imrobot']; ?>为好友；<BR />
				2、发送<span style='color:red'><?php echo $qq_code; ?></span>到QQ机器人<?php echo $this->Config['imrobot']; ?>；<BR /><BR />
				帐号绑定QQ后如需解除，只需将绑定的QQ号码发给机器人即可。
			</div>
			
<?php } ?>
<!-- IM机器人 --><?php } elseif('imjiqiren'==$act) { ?><?php echo $imjiqirenHTML; ?>
			
			<!-- 手机短信 --><?php } elseif('sms'==$act) { ?><?php echo $smsHTML; ?>
		
			<!--QQ微博--><?php } elseif('qqwb'==$act) { ?><form method="post"  action="index.php?mod=qqwb&code=do_modify_bind_info">
...
<?php } ?>
			...
			<tr>
				<td colspan="2">
					您已绑定的QQ微博帐号为：<a target="_blank" href="http://t.qq.com/<?php echo $qqwb_bind_info['qqwb_username']; ?>"><font color="red"><?php echo $qqwb_bind_info['qqwb_username']; ?></font></a>;<br />
					<a onclick="return confirm('您在 <?php echo $this->Config['site_name']; ?> 发表的内容将不再同步到QQ微博，确定解除绑定？');" href="index.php?mod=qqwb&code=unbind">点此可解除绑定</a>
					(<font color="gray">解除绑定后，您在 <?php echo $this->Config['site_name']; ?> 发表的内容将不再同步到QQ微博</font>)
				</td>
			</tr>
<?php } else { ?><tr>
...
<?php } ?>
			...
			<!--新浪微博--><?php } elseif('sina'==$act) { ?><?php echo $profile_bind_message; ?><?php } elseif('share'==$act) { ?><style>.mainL{ _overflow:hidden}</style>
			<div class="friends textB" style="height:260px;">
				<li>
				<p class="vshare">
					<img src="templates/default/images/shareB.gif" border="0" title="分享到<?php echo $shareName; ?>"/>将其他网页内容 <strong>分享到<?php echo $shareName; ?></strong>（复制下面代码，粘贴到其他网页源码中）
				</p>
					<textarea id="invite_url" type="text"  value="" onclick="copyText(this.value);"><a title="分享到<?php echo $shareName; ?>" href="javascript:void(0)" onclick="var d=document,w=window,f='<?php echo $this->Config['site_url']; ?>',l=d.location,e=encodeURIComponent,p=''+e(l.href)+'&t='+e(d.title);if(!w.open(f+'/index.php?mod=share&code=share&url='+p)){l.href=f+'.new'+p;}"><img src="
				<?php echo $this->Config['site_url']; ?>/templates/default/images/shareB.gif" border="0" />分享到<?php echo $shareName; ?></a></textarea>
				</li>
				<li>
					<input type="button" class="save frr" value="复制链接" onclick="copyText($('#invite_url').val(),'inviteURL');" />
				</li>
			</div><?php } elseif('medal'==$act) { ?>
<?php if(is_array($medal_list)) { foreach($medal_list as $val) { ?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			</table><?php } elseif('qmd'==$act) { ?>
<?php if($this->Config['is_qmd']) { ?>
				<script>
					function check_qmd_type(typename)
					{
							...
							if(typename == 'bbs'){
								qmd_value  = "[url=<?php echo $this->Config['site_url']; ?>/index.php?mod=<?php echo $member['username']; ?>][img]<?php echo $member_qmd_img; ?>[/img][/url]";
							}
							if(typename == 'email'){
								;
							}
							if(typename == 'bloge'){
								qmd_value = "<a href='<?php echo $this->Config['site_url']; ?>/index.php?mod=<?php echo $member['username']; ?>' target='_blank'><img border='0' src='<?php echo $member_qmd_img; ?>'/></a>";
							}
							if(typename == 'pic'){
								qmd_value = "<?php echo $member_qmd_img; ?>";
							}
				...
				<dl>
				  ...
				  <dd id="copy_text_con">
					<textarea name="textarea" cols="30" rows="5" id="html_code"><?php echo $member_qmd_img; ?></textarea>
					<p><a id="copyhtml" href="javascript:;"><em>复制代码</em></a>请将代码添加到您的网站</p>
				  </dd>
				</dl>
				
				<div id="skin_images"><img src='<?php echo $member_qmd_img; ?>'/></div>
				<div id="qmd_list"></div>
<?php } else { ?> <p>系统暂时未开启签名档功能。<p>
				 
<?php if(MEMBER_ROLE_TYPE == 'admin') { ?>
