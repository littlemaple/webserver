...
<link href="templates/default/styles/medal.css" rel="stylesheet" type="text/css" />
<script type="text/javascript" src="templates/default/js/medal.js"></script>
...
<div class="main_2b">
  <div class="main_t"><span><?php echo $this->Config['site_name']; ?>勋章</span></div>
	<div class="Menubox2">
	<ul>
...
<?php if(is_array($medallist)) { foreach($medallist as $medal) { ?>
		  
<?php if($medal['is_open'] == 1) { ?>
            <li>
              <div class="ds">
                <div class="ds_img"><img src="<?php echo $medal['medal_img']; ?>"/></div>
                <div class="ds_intro">
                  <p><?php echo $medal['medal_name']; ?></p>
                  <?php echo $medal['medal_depict']; ?> </div>
              </div>
            </li>
...
<?php if(is_array($medallist)) { foreach($medallist as $v) { ?>
	 	$(".yc_<?php echo $v['id']; ?>").mouseover(function(){$(".yc_c_<?php echo $v['id']; ?>").show();});
		$(".yc_<?php echo $v['id']; ?>").mouseout(function(){$(".yc_c_<?php echo $v['id']; ?>").hide();});
...
<?php if(is_array($medallist)) { foreach($medallist as $val) { ?>
        
<?php $medal_type = unserialize($val['conditions']); ?>
<?php if($val['is_open'] == 1) { ?>
          <li class="yc yc_<?php echo $val['id']; ?>"> 
		  <a class="honorlink" href="javascript:void(0);">
		  
<?php if(in_array($val['id'],$medalid)) { ?>
<img src="<?php echo $val['medal_img']; ?>"/>
<?php } else { ?><img src="<?php echo $val['medal_img2']; ?>"/>
<?php } ?>
  </a>
		  <span class="honortitle"><?php echo $val['medal_name']; ?></span>
            <div class="yc_c yc_c_<?php echo $val['id']; ?>">
              <div class="M_box">
                <div class="M_top">
                  <div class="med_img"><img src="<?php echo $val['medal_img']; ?>" /></div>
                  <div class="med_intro">
                    <p><?php echo $val['medal_name']; ?></p>
                    <?php echo $val['medal_depict']; ?> 
					 
<?php if(in_array($val['id'],$medalid)) { ?>
					 （已获得）
					 
<?php } else { ?>  
<?php if(in_array($medal_type['type'],array('topic','reply','invite','tag'))) { ?>
							<a href="index.php?mod=other&code=checkmedal&mids=<?php echo $val['id']; ?>">点亮图标</a>
...
<?php if($medal_type['type'] == 'topic') { ?>
							 ...
							 <?php } elseif($medal_type['type'] == 'reply') { ?> <a href="index.php?mod=topic&code=new" target="_blank">我要发评论</a>
							 <?php } elseif($medal_type['type'] == 'tag') { ?> <a href="index.php?mod=tag&code=<?php echo $medal_type['tagname']; ?>" target="_blank">我要发话题</a>
							 <?php } elseif($medal_type['type'] == 'invite') { ?> <a href="index.php?mod=profile&code=invite" target="_blank">马上去邀请好友</a>
							 <?php } elseif($medal_type['type'] == 'shoudong') { ?>管理员手动发放 |
...
<?php if(is_array($medal_list)) { foreach($medal_list as $img) { ?>
        <img src="<?php echo $img['medal_img']; ?>" style="margin-right:5px; vertical-align:middle" />
        
<?php } } ?>
        <br />
        如果需要帮助，请联系<a href="mailto:<?php echo $this->Config['site_admin_email']; ?>"><?php echo $this->Config['site_admin_email']; ?></a> </div>
    </div>
  </div>
</div>
