 ...
 *
 * @Date 2011-09-16 17:25:29 1749084100 454832283 82742 $
 *******************************************************************/
...
{
	static $S_filelist, $S_lastfile, $S_file, $S_caches;
	...
	else
	{
		...
		$data="<?php if(!defined('IN_PUGOIN')) exit('invalid request'); \r\n\$cache=$data;\r\n?>";
		$fp = @fopen($S_file,"wb");
		flock($fp, LOCK_EX);
		@$len=fwrite($fp,$data);
		flock($fp, LOCK_UN);
...
function cacheclear($handle=false)
{
	if(true === $handle) 
	{
		...
		return @$IoHandler->ClearDir(ROOT_PATH . 'cache/');
	}
	return true;
}
...
function page($total_record, $per_page_num,$url='', $_config = array(), $per_page_nums = "")
{
	global $rewriteHandler;
    $SystemConfig = ConfigHandler::get();
        if(true === IN_PUGOIN_ADMIN && isset($SystemConfig['total_page_default']))
    {
...
class ConfigHandler
{
	...
	function set()
	{
		...
		if($len) $GLOBALS['ConfigHandlerCaches'][$type]=$config;
