 ...
 *
 * @Date 2011-09-16 17:25:38 1770313776 1397765660 27036 $
 *******************************************************************/
...
$sqlfile = './install/pugoin.sql';
$sqldatafile = './install/pugoin_data.sql';
...
if(!defined('INSTALL_LANG') || !function_exists('instmsg') || !is_readable($sqlfile)) {
	exit("Please upload all files to install Pugoin system<br />安装 【蒲公英】 您必须上传所有文件，否则无法继续");
...
<?php
	...
	if(!function_exists('mysql_connect')) {
		...
		else
		{
			if($curr_mysql < '4.1.9')
			{
				$msg .= "<li>您的 MYSQL 版本小于 4.2, 无法使用 Pugoin微博系统。</li>";
				$quit = TRUE;
			}
		}
	}
...
<?php
	...
	if($_POST['saveconfig']) {
		...
		$config['auth_key'] = setconfig(random(16)); 		
		$config['cookie_prefix'] = setconfig('Pugoin_' . random(6) . '_'); 		
		...
		$config['db_table_prefix'] = setconfig($_POST['db_table_prefix'] ? $_POST['db_table_prefix'] : 'pugoin_' . random(6) . '_');
		...
		$_rewrite=array (
		  ...
		  'value_replace_list' =>
		    ...
		    'mod' =>
		    array (
				'topic' => array_rand(array('miniblog'=>1,'myblog'=>1,'blog'=>1,'topic'=>1,)),
				'tag' => array_rand(array('keyword'=>1,'channel'=>1,'class'=>1,'tag'=>1,)),
				'profile' => array_rand(array('my'=>1,'profile'=>1,)),
				'member' => array_rand(array('user'=>1,'member'=>1,)),
		    ),
		  ),
		  'gateway' => '',
		);
			...
			if(!$write_error) {
				...
				if(!isset($config['link_list']['biniu'])) {
					$biniu_link_list = array(
						...
						array (
						    'name' => _getaarrayrandval(array('团购源码下载','手拉手团购系统','团购系统','免费团购系统','团购程序','团购网站源码','开源团购系统',)),
						    'url' => 'http:/'.'/www.dream3.cn',
						),
						array (
						    'name' => _getaarrayrandval(array('免费微博系统','开源微博系统','开源微博程序','微博程序','微博系统','微博网站源码','微博源码下载','微博',)),
						    'url' => 'http:/'.'/www.pugo.in',
						),
	...
	$config['db_table_prefix'] = 'pugoin_';
?>
<tr><td align="center">
<table width="80%" cellspacing="1" bgcolor="#000000" border="0" align="center">
<tr bgcolor="#3A4273"><td style="color: #FFFFFF; padding-left: 10px" width="32%"><?php echo $lang['tips_message']; ?></td>
...
<?php
	...
	if(!@mysql_connect($config['db_host'], $config['db_user'], $config['db_pass'])) {
		...
		$sqlarray = array(
				'createtable' => 'CREATE TABLE `pugoin_test` (`test` TINYINT (3) UNSIGNED)',
				'insert' => 'INSERT INTO `pugoin_test` (`test`) VALUES (1)',
				'select' => 'SELECT * FROM `pugoin_test`',
				'update' => 'UPDATE `pugoin_test` SET `test`=\'2\' WHERE `test`=\'1\'',
				'delete' => 'DELETE FROM `pugoin_test` WHERE `test`=\'2\'',
				'droptable' => 'DROP TABLE `pugoin_test`'
			);
...
$extrasql = <<<EOT
replace into `pugoin_members`(`uid`,`username`,`nickname`,`password`,`email`,`role_id`,`role_type`,`invitecode`) values (1,'$username','$username','$password','$email',2,'admin','{$invitecode}');
replace into `pugoin_memberfields`(`uid`,`nickname`) values('1','$username');
EOT;
