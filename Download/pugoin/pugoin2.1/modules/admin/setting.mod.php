...
class ModuleObject extends MasterObject
{
	...
	function Execute()
	{
		...
		switch($this->Code)
		{
			...
			case 'modify_onlinepay':
				$this->ModifyOnlinepay();
				break;
			case 'do_modify_onlinepay':
				$this->DoModifyOnlinepay();
				break;
				
			case 'modify_smsplugin':
				$this->ModifySmsplugin();
				break;
			case 'do_modify_smsplugin':
				$this->DoModifySmsplugin();
				break;
	...
	function DoModifySeccode()
	{
		...
		if($seccode['type']) {
			...
			if ($ch = false) {
				$this->Messager('开启中文图'.'片验证码前，需要将包含完'.'整中文汉字的 TTF 中'.'文字体文件上传到 ima'.'ges/fo'.'nts/ch 目'.'录下，<a hr'.'ef=ht'.'tp:/'.'/ai'.'j'.'uhe'.'.net/gro'.'up_'.'thread/'.'view/id-'.'802 target="_blank"><font '.'color=r'.'ed>点此查'.'看说明及下载'.'蒲公英官方提'.'供的中文字体</f'.'ont></a>',null);
			}
		}
	...
	function ModifyOnlinepay()
	{
		//echo 'aaaaaaaaaaaaaaaa'; // 可以到达
		$onlinepay = ConfigHandler::get('onlinepay');
		$action = "admin.php?mod=setting&code=do_modify_onlinepay";
		include($this->TemplateHandler->Template('admin/setting_onlinepay'));
	}
	function DoModifyOnlinepay()
	{
	
		ConfigHandler::set('onlinepay',$_POST['onlinepay']);

		$this->Messager("设置成功{$msg}");

	}
	
	function ModifySmsplugin()
	{
		//echo 'aaaaaaaaaaaaaaaa'; // 可以到达
		$smsplugin = ConfigHandler::get('smsplugin');
		$action = "admin.php?mod=setting&code=do_modify_smsplugin";
		
		include($this->TemplateHandler->Template('admin/setting_smsplugin'));
	}
	function DoModifySmsplugin()
	{
	
		ConfigHandler::set('smsplugin',$_POST['smsplugin']);
