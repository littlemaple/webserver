...
class ModuleObject extends MasterObject
{
	...
	//error_log("dolist:"."<br>",3,'d:\www\sql_error.html');
		extract($this->Get);
		extract($this->Post);
		$options = array();
	...
	function DoAdd()
	{
		...
		$shoppingurl =  trim($this->Post['shoppingurl']);
		
		$videoid = max(0, (int) $this->Post['videoid']);
		
		$longtextid = max(0, (int) $this->Post['longtextid']);

		//shoppingid从增加的topic_shopping表中获取
		//这里判断是否有shoppingurl，如果有，则调用topic.logic.php里的方法解析站点url并且存入topic_shopping表
		
		
		$shoppingid = 0;
		if (!empty($imageid) && !empty($shoppingurl)) {
			// Doshopping 判断是否有图片和网址，如果有，则添加shopping
			//$shoppingid = $this->Doshopping($shoppingurl);
			
			Load::logic('shopping');
			$ShoppingLogic = new ShoppingLogic($this);
			
			//shopping logic 生成shopping来给topic更新
			$shoppingid = $ShoppingLogic-> get_shoppingid($shoppingurl,0,MEMBER_ID);
			
		}
		
		//这里分析分类classid
		$classid =  $this->TopicLogic->GetClassId($content);
	...
	{
		$tid = (int) ($this->Post['tid'] ? $this->Post['tid'] : $this->Get['tid']);
		$tid = $modify_tid = max(0, $tid);
		
		if($tid < 1)
		{
			js_alert_output('分享ID 错误');
		}
		...
		if(MEMBER_ROLE_TYPE != 'admin')
		{
	        ...
	        {
	            response_text("您没有权限编辑该分享");
	        }
        
    		if($topiclist['replys'] >= 1 || $topiclist['forwards'] >= 1 )
    		{
    			response_text("分享已被评论或者转发,不能编辑");
    		}
		...
		//判断是否有图片，如果有则读取
		$m_shoppingid =(int) $row['shoppingid'];
		
		if( $m_shoppingid > 0 ){
			 $row = $this->DatabaseHandler->FetchFirst("select * from ".TABLE_PREFIX."topic_shopping where `id`='$m_shoppingid'");
        	 $topiclist['shoppingurl'] = $row['shopping_url'];
		}
	...
	function Do_ModifyTopic()
	{
		...
		if(MEMBER_ROLE_TYPE != 'admin')
		{
			if(MEMBER_ID != $topiclist['uid'])
	        {
	            js_alert_output("您没有权限编辑该分享");
	        }
        
    		if($topiclist['replys'] >= 1 || $topiclist['forwards'] >= 1 )
    		{
    			js_alert_output("分享已被评论或者转发,不能编辑");
    		}
    		
				        if($this->Config['topic_modify_time'] && (($topiclist['addtime'] ? $topiclist['addtime'] : $topiclist['dateline']) + ($this->Config['topic_modify_time'] * 60) < time()))
	        {
	        	js_alert_output("分享已超出可编辑时间了");
	        }
		}
		...
		$shoppingurl = $this->Post['shoppingurl'];
		
		$return = $this->TopicLogic->Modify($tid,$content,$imageid,$shoppingurl);
	...
	function DoVideo()
	{
        ...
        $return_content = $return['title'] ? $return['title'] : '分享链接 '.$url;
	...
	function DoShopping($url)
	{
        ...
        $return = $this->TopicLogic->_parse_shopping($url);		
		
		if ($return) 
        {
           	$shopping_link 		= $return['id'];
           	$shopping_hosts 	= $return['host'];
           	$shopping_url		= $return['url'];
			$shopping_price		= $return['price'];
            $shopping_img = '';
            $shopping_img_url = "";
		}else{
			$shopping_link 		= '';
		...
		values 
		('".MEMBER_ID."','".''."','".MEMBER_NAME."','".$shopping_hosts."','".$shopping_link."','".$shopping_url."','".$shopping_img."','".$shopping_img_url."','".$shopping_price."','".$timestamp."')";
		$this->DatabaseHandler->Query($sql);
	...
	function Dig()
	{
		...
		//error_log("dig:"."<br>",3,'d:\www\sql_error.html');
		...
		$topic = $this->TopicLogic->Get($tid);
		if (!$topic) {
			js_alert_output("分享已经不存在了");
		}
		
		
		$return = $this->TopicLogic->InsertTopicDig($tid,$uid);
		
		if($return === true){
			$digs = $topic[digs] + 1;
			$response .= $digs;
		}else{
			js_alert_output($return);
		}
