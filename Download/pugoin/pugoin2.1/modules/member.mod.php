...
class ModuleObject extends MasterObject
{
	...
	function Register()
	{
		...
		if ($this->Config['invite_enable']) {
			if(!$this->Code) {
				...
				$this->Messager("非常抱歉，本站目前需要有邀请链接才能注册,请加QQ：2432241898，或发送邮到到：2432241898@qq.com。",null);
			} else {
		...
		(
			'1'=>array('name'=>"正在上大学",'value'=>"2"),
			'2'=>array('name'=>"已经工作了",'value'=>"1"),
			'3'=>array('name'=>"正在上中学",'value'=>"3"),
			'4'=>array('name'=>"是其他身份",'value'=>"0"),
		);
		
		Load::lib('form');
		$work_type_select=FormHandler::Select('work_type',$work_type_list,null,"onchange=\"changeWorkType();\"");
	...
	{		
		...
		if($this->_sms_register())
		{
			...
			if($noemail)
			{
				$this->Post['email'] = $sms_bind_num . '@pugoin.com';
			}
		}
		...
		$work_type = $this->Post['work_type'];
		
		//判断城市大学匹配
		if((int)$work_type == 2 && empty($this->Post['college'])){ 
			$this->Messager("请刷新注册页面重新选择您所读的大学。",null);
		}
		
		if((int)$work_type != 2 && empty($this->Post['city'])){ 
			$this->Messager("请刷新注册页面重新选择您所在区域。",null);
		}
		...
		$datas = array();	
		$datas['uid'] = $uid;	
		$datas['work_type'] = $work_type;	
		$datas['province'] = $this->DatabaseHandler->ResultFirst("select name from ".TABLE_PREFIX."common_district where id = ".$this->Post['province']);
		if((int)$work_type != 2){ 		
			$datas['city'] = $this->DatabaseHandler->ResultFirst("select name from ".TABLE_PREFIX."common_district where id = ".$this->Post['city']);		
			if($this->Post['area']){
				$datas['area'] = $this->DatabaseHandler->ResultFirst("select name from ".TABLE_PREFIX."common_district where id = ".$this->Post['area']);		
			}
			if($this->Post['street']){
				$datas['street'] = $this->DatabaseHandler->ResultFirst("select name from ".TABLE_PREFIX."common_district where id = ".$this->Post['street']);		
			}
		}else{
			$datas['college_id'] = $this->Post['college'];		
			$datas['college_name'] = $this->DatabaseHandler->ResultFirst("select name from ".TABLE_PREFIX."common_college where id = ".$this->Post['college']);		
		}
		...
		if ($this->Config['invite_enable']) {
			...
			//如果这时候邀请数量达到要求，且该用户如果为待验证用户，则更新改用户的等级为普通用户
			$sql = "select `invite_count`,email from `".TABLE_PREFIX."members` where `uid`='{$u}' and role_id=5";
			$query = $this->DatabaseHandler->Query($sql);
			$inviter = $query->GetRow();
			if($inviter) {
				$invite_count = $inviter['invite_count'];
				if($this->Config['invite_active_count'] <= $invite_count)
				{
					
					$sql = "update `".TABLE_PREFIX."members` set `role_id`=3 where `uid`='{$u}'";
					//die("1111".$sql);
					$this->DatabaseHandler->Query($sql);
					//同时给原来用户发送mail提示注册成功
					...
					send_member_account_activ_mail($uid,$inviter['email']);
				}
				//die("ddd:".$invite_count);
			}
		...
		if($this->Config['reg_email_verify']!='1')
		{
			...
			$this->Messager(null,'index.php?mod=member&code=stepInvite&nickname='.$nickname,0);
	...
	{	
		...
		include($this->TemplateHandler->Template('member_stepInvite'));
	}
	
	function DoStepInvite()
	{
	}
