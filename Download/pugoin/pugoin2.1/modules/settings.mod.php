...
class ModuleObject extends MasterObject
{
	...
	function Main()
	{
		...
		$act_list = array('base'=>'我的资料','face'=>'我的头像','secret'=>'修改密码','user_medal'=>'我的勋章','exp'=>'用户等级','user_tag'=>array('name'=>'我的标签','link_mod'=>'user_tag',),);
		...
		elseif('base' == $act)
		{
			...
			$work_type_select=$FormHandler->Select('work_type',$work_type_list,$member['work_type'],"onchange=\"changeWorkType();\"");
			...
			if($province_id){
				if($member['college_name']){
					$hid_college = $this->DatabaseHandler->ResultFirst("select id from ".TABLE_PREFIX."common_college where name = '$member[college_name]' and upid = $province_id");				}
	...
	function DoModifyProfile()
	{
		...
		$work_type = trim($this->Post['work_type']);

		$province = trim($this->DatabaseHandler->ResultFirst("select name from ".TABLE_PREFIX."common_district where id = ".$this->Post['province'])); 		
		if($work_type == 2){
			$college = $this->Post['college'];
			if($this->Post['college']){
				$college_name = trim($this->DatabaseHandler->ResultFirst("select name from ".TABLE_PREFIX."common_college where id = ".$this->Post['college']));		
			}
		}else{
			$city = trim($this->DatabaseHandler->ResultFirst("select name from ".TABLE_PREFIX."common_district where id = ".$this->Post['city']));		
			if($this->Post['area']){
				$area = trim($this->DatabaseHandler->ResultFirst("select name from ".TABLE_PREFIX."common_district where id = ".$this->Post['area']));		
			}
			if($this->Post['street']){
				$street = trim($this->DatabaseHandler->ResultFirst("select name from ".TABLE_PREFIX."common_district where id = ".$this->Post['street']));		
			}
			$college = 0;
			$college_name = '';
		}
		...
		$aboutme = strlen(($aboutme = trim(strip_tags($this->Post['aboutme'])))) > 255 ? substr($aboutme,0,254) . ' ' : $aboutme;
		...
		if($work_type == 2){
			if (!$province || !$college)
			{
				$this->Messager("省所在地和学校不能为空，请返回修改",-1);
			}
		}else{
			if (!$province || !$city)
			{
		...
		$arr = array (
			'work_type' => addslashes($work_type),
			'college_id' => addslashes($college),
