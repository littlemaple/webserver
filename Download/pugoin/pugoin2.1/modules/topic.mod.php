...
class ModuleObject extends MasterObject
{
	...
	{	
		...
		if ('myhome'==$params['code']) {
			...
			if ($member['uid'] == MEMBER_ID) {
				...
				$title = '我的首页';
					...
					while ($row = $query->GetRow()) {
						...
						$this->Messager("没有设置用户，无法查看这个组的分享",'index.php?mod=topic&code=group&gid='.$gid);
					}
					$active[$gid] = "current";
				} else {
			...
			if ($this->Get['type']) {
			 ...
			 if($this->Get['type'] == 'shopping'){
				...
				$d = isset($d_list[$this->Get['d']]) ? $this->Get['d'] : 30;
				$time = $d * 86400;
				$dateline = time() - $time;
				
				//暂时采用最新分享的设置，待补充 @ToDo
				$per_page_num = 15;	
				$query_link = "index.php?mod=".$member['username']."&type=shopping" ;
				...
				$options = array(
					...
					'page_url' => $query_link,
					'type' => 'first',
					'where' => " `imageid` > 0 and uid='".$params['uid']."' ",
					'order' => " dateline DESC  ",
				);
			...
			if ($member['uid'] != MEMBER_ID) {
				...
				$title = '我的分享';
				$this->MetaKeywords ="{$member['nickname']}的分享";
			}
		...
		if ($fg_code == 'hisblog'){
			...
			include($this->TemplateHandler->Template('topic_hisblog'));
	...
	function View()
	{
		...
		//更新计数器hits
		$this->TopicLogic->UpdateHits($this->ID);
		...
		$this->Title = cut_str(strip_tags($topic_info['content']),50)." - {$member['nickname']}的分享";
	...
	function Follow()
	{
		...
		$this->Title = "{$member['nickname']}关注的分享";
	...
	function Fans()
	{
		...
		if($uids) {
			...
			$_list = $this->TopicLogic->GetMember($uids,"`uid`,`ucuid`,`username`,`face_url`,`face`,`province`,`city`,`college_name`,`fans_count`,`topic_count`,`validate`,`gender`,`face`,`nickname`,`level`");
			foreach ($uids as $uid) {
				if(isset($_list[$uid])) {
	...
	function _guestIndex()
	{
		...
		include($this->TemplateHandler->Template('topic_index_login'));
	}
	...
	function Shopping()
	{
		...
		if('shopping' == $this->Code)
		{
			...
			if($cls){
				$subclassname = $this->TopicLogic->GetSubClassName($cls);
			...
			if($order =='dig'){
				...
				$sqlorder = ' dateline DESC';
			}
			
			if($cls){
				// 当前分类的名称
				$query_link = "index.php?mod=" . ($_GET['mod_original'] ? get_safe_code($_GET['mod_original']) : $this->Module) . ($this->Code ? "&amp;code={$this->Code}" : ""). ($this->cls ? "&amp;cls={$this->cls}" : ""). ($this->order ? "&amp;order={$this->order}" : "");
				$options = array(
					'perpage' => $per_page_num,
					'page_url' => $query_link,
					'type' => 'first',
					'where' => " `imageid` > 0 and classid= ".$cls,
					'order' => $sqlorder,
				);
				$info = $TopicListLogic->get_data($options);
				$topics = array();
			...
			{
				$query_link = "index.php?mod=" . ($_GET['mod_original'] ? get_safe_code($_GET['mod_original']) : $this->Module) . ($this->Code ? "&amp;code={$this->Code}" : ""). ($this->order ? "&amp;order={$this->order}" : "");
				$options = array(
					'perpage' => $per_page_num,
					'page_url' => $query_link,
					'type' => 'first',
		...
		// 分类列表
		$class_list = $this->TopicLogic->GetClassList();
				

		$this->Title = $title;

		if($cls){
			include($this->TemplateHandler->Template('topic_index_shopping_class'));
		}
		else
		{
			include($this->TemplateHandler->Template('topic_index_shopping'));
		}
		...
		$TopicListLogic = new TopicListLogic();
			...
			if($this->ShowConfig['topic_new']['topic'] > 0) {
				$options = array(
					...
					'page_url' => $query_link,
					'type' => 'first',
					'where' => " `imageid` > 0 and 'uid'='".MEMBER_ID."' ",
					'order' => " dateline DESC  ",
				);
				$info = $TopicListLogic->get_data($options);
				$topics = array();
