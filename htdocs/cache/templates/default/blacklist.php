...
<!--此处三栏-->
	...
	<div class="mainL">
	  <div class="main_t"><span>黑名单</span></div>
		<div class="searchBox">
		 <span style="font-size:14px; padding:0 15px;">被加入黑名单的用户将无法关注你、评论你。如果你已经关注他，也会自动解除关系。</span>
		 <ul class="followList">
		  
<?php if($total_record) { ?>
共有<?php echo $total_record; ?>位用户
<?php } ?>
  
<?php if($member_list) { ?>
		  
<?php if(is_array($member_list)) { foreach($member_list as $val) { ?>
		  <li id="follow_user_<?php echo $val['uid']; ?>">
			<div class="fBox_R">
			   <div class="fBox_l"><a href="index.php?mod=<?php echo $val['username']; ?>"><img onerror="javascript:faceError(this);" src="<?php echo $val['face']; ?>" /></a></div>
			   <span><a href="index.php?mod=<?php echo $val['username']; ?>"><?php echo $val['nickname']; ?></a></span>
			   <p><span> 关注 <?php echo $val['follow_count']; ?> 人 |被 <?php echo $val['fans_count']; ?> 人关注 | 微博 <?php echo $val['topic_count']; ?> 条</span></p>
			</div>
			<!--拉黑-->
			 <div class="fBox_R2" style="width:180px;">
				<div class="blacklist">
					<!--他在你的黑名单里<a href="javascript:void(0)" onclick="follower_choose(<?php echo $val['uid']; ?>,'<?php echo $val['nickname']; ?>','del','blacklist');"> [移除黑名单] </a>-->
					他在你的黑名单里<a href="javascript:void(0)" onclick="DelBlackListDialog(<?php echo $val['uid']; ?>);"> [移出黑名单] </a>
				</div> 
			</div>
			<!--拉黑-->
...
<?php if($val['uid'] == $t_val['uid']) { ?>
					  <span style="color:#CCCCCC;">最近更新：<?php echo $t_val['dateline']; ?><?php echo $t_val['tid']; ?>  </span>
