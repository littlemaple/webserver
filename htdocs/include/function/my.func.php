 ...
 *
 * @Date 2011-09-16 17:25:30 1595635739 1389042680 2770 $
 *******************************************************************/
...
function send_member_account_activ_mail($uid,$email)
{
	if(1 > ($uid = (int) $uid)) return false;
	if(!($email = trim($email))) return false;
	...
	$email_message="您好：
您收到这封邮件，是因为您在{$sys_config['site_url']}网站注册的帐号已被激活。
如果您有进行上述操作，请忽略这封邮件。您不需要退订或进行其他进一步的操作。
------------------------------------------------------
您只需点击下面的链接登录：
{$sys_config['site_url']}/index.php
...
";
	...
	$send_result = send_mail(
		$email,
		" [{$sys_config['site_name']}]提示您的帐号已被激活",
		$email_message,
		$sys_config['site_name'],
		$sys_config['site_admin_email'],
		array(),
