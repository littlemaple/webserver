...
function _send_mail_by_smtp($email_to,$email_subject,$email_message,$smtp_config='',$html=true) {
	...
	$headers = "From: $email_from{$maildelimiter}X-Priority: 3{$maildelimiter}X-Mailer: pugoin ".SYS_VERSION."{$maildelimiter}MIME-Version: 1.0{$maildelimiter}Content-type: text/".($html ? 'html' : 'plain')."; charset=$charset{$maildelimiter}Content-Transfer-Encoding: base64{$maildelimiter}";
	...
	fputs($fp, ($mail['auth'] ? 'EHLO' : 'HELO')." pugoin\r\n");
	$lastmessage = fgets($fp, 512);
...
$result=send_mail('foxis@qq.com',"主题测试","内容测dddd试<BR><BR><BR><BR>","pugoin",'foxis@qq.com',$attachment_list);
if($result==true)
{
	echo "发送成功";
}
