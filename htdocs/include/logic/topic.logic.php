...
class TopicLogic
{
    ...
    function Add($content, $totid = 0, $imageid = 0, $from = 'web', $type = "first", $uid = 0, $item = '', $item_id = 0)
    {
		...
		//增加shopingid
		$data['shoppingid'] = $shoppingid = max(0, (int)$shoppingid);
		
		//增加classid
		$data['classid'] = $classid = max(0, (int)$classid);
		...
		//更新topic_shopping的tid字段
		if ($data['shoppingid'] > 0)
        {
            $sql = "update `" . TABLE_PREFIX . "topic_shopping` set `tid`='{$tid}' where `id`='{$data['shoppingid']}'";
            $this->DatabaseHandler->Query($sql);
        }
    ...
    function Modify($tid,$content,$imageid=0,$shoppingurl='')
    {
        $sql_sets = array();
        ...
        //判断shopping的事情
		Load::logic('shopping');
        $ShoppingLogic = new ShoppingLogic($this);
		//shopping logic

        $shoppingid = $ShoppingLogic->get_shoppingid($shoppingurl,$tid,MEMBER_ID);
		
		//end shopping
		//这里分析分类classid
		$classid =  $this->GetClassId($content);
		
		$sql_sets['classid'] = "`classid`='$classid'";
    ...
    function Make($topic, $actors = array())
    {
        ...
        if (is_numeric($highlight) && $highlight == $topic['tid'])
        {
            $topic['username'] = "<b>{$topic['username']}</b>";
            $topic['dateline'] = "<b>{$topic['dateline']}</b>";
            $topic['content'] = "<b><font color=red>{$topic['content']}</font></b>";

            unset($_GET['highlight']);
        }
        ...
        if ($topic['videoid'] > 0)
        {
			...
			//die($topic['VideoHosts'].">>".$topic['videoid']);
		...
		//增加shopping
		if ($topic['shoppingid'] > 0)
        {
            $sql = "select `id`,`shopping_hosts`,`shopping_link`,`shopping_img`,`shopping_img_url`,`shopping_url`,`shopping_price` from `" .
                TABLE_PREFIX . "topic_shopping` where `id`='" . $topic['shoppingid'] . "' ";
            $query = $this->DatabaseHandler->Query($sql);
            $topic_shopping = $query->GetRow();

            $topic['ShoppingID'] = $topic_shopping['id'];
            $topic['ShoppingHosts'] = $topic_shopping['shopping_hosts'];
            $topic['ShoppingLink'] = $topic_shopping['shopping_link'];
			$topic['ShoppingUrl'] = $topic_shopping['shopping_url'];
			$topic['ShoppingPrice'] = $topic_shopping['shopping_price'];
        
            if ($topic_shopping['shopping_hosts'])
            {
				//将taobao.com替换为taobao_com
				$shoppingTypeLogoName = strtr($topic_shopping['shopping_hosts'],".","_") ;
                $topic['ShoppingTypeLogo'] = $this->Config['site_url'] . '/images/shopping/'.$shoppingTypeLogoName.'.gif';
            }else{
				$topic['ShoppingTypeLogo'] = $this->Config['site_url'] . '/images/shopping/unresolving.gif';
			}
        }
    ...
    function MakeMember($row)
    {
		...
		*/
		...
		if (isset($row['province']))
        {
			...
			if ( isset($row['college_name']) && !empty($row['college_name'])){
            	$row['from_area'] = "{$row['province']} {$row['college_name']}";
			}
        }
    ...
    function _syn_to_sina($data = array())
    {
        if ($this->Config['sina_enable'] && $data && $data['uid'] > 0 && $data['tid'] > 0 && 'sina'!=$data['from'] &&
            sina_weibo_init($this->Config) && sina_weibo_bind($data['uid']) && !$GLOBALS['imjiqiren_sys_config']['imjiqiren']['sina_update_disbale'])
        {
            ...
            {
				if( TRUE===IN_PUGOIN_INDEX || TRUE===IN_PUGOIN_AJAX || TRUE===IN_PUGOIN_ADMIN )
				{
    ...
    function _syn_to_qqwb($data = array())
    {
        if ($this->Config['qqwb_enable'] && $data['uid'] > 0 && $data['tid'] > 0 && 'qqwb'!=$data['from'] &&
            qqwb_init($this->Config) && qqwb_bind($data['uid']) && !$GLOBALS['imjiqiren_sys_config']['imjiqiren']['qqwb_update_disbale'])
        {
    ...
    {        
        ...
        $vhconfs = array(
			//http://v.youku.com/v_playlist/f16176975o1p0.html 解析列表页面
			//http://v.youku.com/v_show/id_XMzEwOTIxMTYw.html  对应p
        	...
        	//http://www.tudou.com/playlist/p/l13672133.html
			"tudou.com" => array('p'=>"/\/([\w\d\-\_]+)\/" . "*$/",'c'=>'gbk','tp'=>"/\<title\>(.+?)(?:[\-\_].*?)?\<\/title\>/",'ip'=>"/bigItemUrl\s*[\=\:]\s*[\"\']([^\"\']+?)[\"\']/", 'ppp'=>1, 'gzip'=>1,),
        	//http://v.ku6.com/special/show_5795924/obRqctmZx9t5PwxF.html
			//http://v.ku6.com/show/x_9V1x96yUV1AYk_.html
	...
	//parse class
	function GetClassId($content)
	{ 
	    
        $classid = 0;        
        
		$query = DB::query("SELECT upid,name FROM ".DB::table('common_class_tag')." order by id");
		
		while ($row = $query->GetRow())
		{
			$pos = strpos($content, $row['name']);
	...
	//update hits
	function UpdateHits($tid){
		$sql = "update `" . TABLE_PREFIX . "topic` set hits = hits + 1  where `tid`='{$tid}'";
        $this->DatabaseHandler->Query($sql);
	}
	//end update hits
    ...
    {
        $sql = "select `id`,`name`  from `" . TABLE_PREFIX ."common_class` where upid = 0 ORDER BY `list` DESC";
        $query = $this->DatabaseHandler->Query($sql);
        $class_list = array();
        while ($row = $query->GetRow())
        {
            //$classlist[$row['uid']] = $row['id'];
			$sql = "select `id`,`name`  from `" . TABLE_PREFIX ."common_class` where `upid`='{$row['id']}' ORDER BY `list` DESC";
        	$subquery = $this->DatabaseHandler->Query($sql);
			
			$sub_class = array();
			
			$sub_class['classid'] = $row['id'];
			$sub_class['classname'] = $row['name'];
			...
			$sub_class_list = array();
			while ($subrow = $subquery->GetRow())
			{
				$sub_class_list[$subrow['id']] = $subrow;
			}
			$sub_class['sub_class_list'] = $sub_class_list;
			
			$class_list[$row['id']] = $sub_class;
        }
	...
	//得到分类名称
	function GetSubClassName($cid)
    {
		$subclassname = '';
        $sql = "select `id`,`name`  from `" . TABLE_PREFIX ."common_class` where upid > 0 and `id`='{$cid}'";
        $subclass = $this->DatabaseHandler->FetchFirst($sql);
		if($subclass)
		{
	...
	//得到分类名称
	function InsertTopicDig($tid,$uid)
    {
		$timestamp = time();
        $sql = "select `id`  from `" . TABLE_PREFIX ."topic_dig` where  `tid`='{$tid}' and `uid`='{$uid}'";
        $topicDig = $this->DatabaseHandler->FetchFirst($sql);
		if($topicDig)
		{
			return "您已经参与过了";
		}
		
		$sql = "insert into `" . TABLE_PREFIX . "topic_dig`(`tid`,`uid`,`dateline`) values('{$tid}','{$uid}','{$timestamp}')";
        $this->DatabaseHandler->Query($sql);
		
		$sql = "update `" . TABLE_PREFIX . "topic` set digs = digs + 1  where `tid`='{$tid}'";
        $this->DatabaseHandler->Query($sql);
