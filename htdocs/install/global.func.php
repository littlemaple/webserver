 ...
 *
 * @Date 2011-09-16 17:25:37 1730068447 731820065 12223 $
 *******************************************************************/
...
function instheader() {
	...
	echo "<html><head>".
		...
		"<title>Pugoin Installation Wizard $version </title>".
		...
		"<table width=\"95%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" bgcolor=\"#FFFFFF\" align=\"center\"><tr><td>".
          	...
          	"<td class=\"install\" height=\"30\" valign=\"bottom\"><font color=\"#FF0000\">&gt;&gt;</font> Pugoin $version&nbsp;$lang[install_wizard] ".
...
function instfooter() {
	...
	echo "<tr><td><hr noshade align=\"center\" width=\"100%\" size=\"1\"></td></tr>".
        	"<tr><td align=\"center\">".
            	"<b style=\"font-size: 11px\">Powered by <a href=\"http:/"."/pugo.in\" target=\"_blank\">蒲公英(pugo.in) $version".
          	"</a> &nbsp; Copyright &copy;  2010 - 2011</b><br /><br />".
...
function runquery($sql) {
	...
	$sql = str_replace("\r", "\n", str_replace('`pugoin_',"`" . $config['db_table_prefix'], $sql));
	$ret = array();
	$num = 0;
	foreach(explode(";\n", trim($sql)) as $query) {
		$queries = explode("\n", trim($query));
...
{
	if(function_exists('var_export')==false)die("PHP版本不能低于4.2.0");
	if(is_writable($file)==false)die("配置文件{$file}不可写，请修改其属性为0777");
	ksort($config);
    ...
    {
        @copy($file,str_replace('/setting/','/backup/setting/',$file));
    }
...
function install_request($post=array(),&$error) {
	...
	$server_url = "http:/"."/www.pugo.in/server.php";
