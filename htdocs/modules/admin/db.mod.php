...
class ModuleObject extends MasterObject
{
	...
	{	
		...
		if (!$backupdir) {			
			...
			foreach ($_f_list as $_k=>$_f) {
				...
				if(!in_array($ext,array('sql','zip',)) || 'pugoin.sql'==basename($_f)) {
					unset($_f_list[$_k]);
			...
			foreach($exportlog as $info) {		
				...
				($info['version'] != SYS_VERSION ? " onclick=\"return confirm('导入和当前 Pugoin 版本不一致的数据极有可能产生无法解决的故障，您确定继续吗？');\"" : '').">[导入]</a></td>\n");
			}
		}
	...
	function DoImport()
	{
		...
		if($dumpinfo['method'] == 'multivol') {
			...
			$this->Messager('数据文件非 Pugoin 格式，无法导入。');
		}
	...
	function DoExport()
	{
		...
		if($method == 'multivol') {
			...
			if(trim($sqldump)) {
				$sqldump = "$idstring".
				"# <?exit();?>\n".
				"# Pugoin Multi-Volume Data Dump Vol.$volume\n".
				"# Version: Pugoin ".SYS_VERSION."\n".
				"# Time: $time\n".
				"# Type: $type\n".
				"# Table Prefix: $tablepre\n".
				"#\n".
				"# Pugoin Home: http:\/\/www.pugo.in\n".
				"# Please visit our website for newest infomation about Pugoin\n".
				"# --------------------------------------------------------\n\n\n".
				"$setnames".
				$sqldump;
