...
class ModuleObject extends MasterObject
{
	...
	function Add() {
		...
		$share = $query->GetRow();
				...
				$style = @unserialize($share['topic_style']);
		
	    $style['topic_name'] = '微博';
	    $style['topicname_color'] = '#626262';
		$style['title_color'] = '#ffffff';
		$style['border_color'] = '#ebebeb';
		$style['border_siz'] = '1';
		...
		$style['link_color'] = '#7d7d7d';
		$style['text_color'] = '#626262';
		
				$show = @unserialize($share['show_style']);
	
		$show['topic_charset'] = 'gbk';
		$show['topic_string'] = '100';
		$show['topic_limit'] = '20';
		$show['topic_up'] = 'topic_up_zd';
		
				$module = @unserialize($share['condition']);
		
		$module['face'] = 'face';
		$module['nickname'] = 'nickname';
		$module['content_link'] = 'content_link';
	...
	function Delete()
	{
		...
		$sql = "delete from `".TABLE_PREFIX."share` where `id` = '{$ids}'";
		$this->DatabaseHandler->Query($sql);
