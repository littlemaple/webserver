...
class ModuleObject extends MasterObject
{
	...
	function Main()
	{
		...
		$collegeprovince = $this->Get['collegeprovince'];
		$hid_college = $this->Get['hid_college'];
		if($collegeprovince && $collegeprovince != 0){
			$query = $this->DatabaseHandler->Query("select * from `".TABLE_PREFIX."common_college` where upid = $collegeprovince order by list ");
			while ($rs = $query->GetRow()){
				if($hid_college == $rs['id']){
					$show .= "<option value={$rs['id']} selected>{$rs['name']}</option>\t\n";
				}else{
					$show .= "<option value={$rs['id']} >{$rs['name']}</option>\t\n";
				}
