...
{	
	...
	function Refresh($num_list=1,$check_num=0)
	{
		...
		$type_array = array('follow','tag','user_tag','area');
		...
		elseif($refresh_type == 'area')
		{	
			$member_info = DB::fetch_first("select `uid`,`province`,`city`,`college_name` from ".DB::table('members')." where `uid` = '{$my_id}' ");
			
			if(empty($member_info['college_name'])){
				$refresh_type = 'city';
				$query = DB::query("select `uid` from ".DB::table('members')." where `city` = '{$member_info['city']}' and `uid` != '{$my_id}' ");
				
			}else{
				$refresh_type = 'college';
				$query = DB::query("select `uid` from ".DB::table('members')." where `college_name` = '{$member_info['college_name']}' and `uid` != '{$my_id}' ");
		...
		if($uids)
		{
			...
			//此处过滤掉头像为空的部分and face <> ''
					    $condition = "where `uid` in ('" . implode("','", $list_uids) . "') and face <> '' order by `uid` desc limit 0,{$rand_number}";		 
	...
	function RandomUser()
	{
		...
		{	
			...
			$query = DB::query("SELECT `uid` FROM ".DB::table('members')." where role_id=3 and face <> ''");
			//$query = DB::query("SELECT `uid` FROM ".DB::table('members')." ");
			
			while ($value = DB::fetch($query))
			{
				$normaluser_id[] = $value['uid'];
			}
			
			$temp_uids = array_unique($normaluser_id);
			//print_r($temp_uids);
			
			if(count($temp_uids) <= $num_list){
				$uids = $temp_uids;
			}else{
				$uidIndex = array_rand($temp_uids,$num_list);
				foreach ($uidIndex as $uidItem)
				{
					$uids[] = $temp_uids[$uidItem];
				}
			}
        ...
        include($this->TemplateHandler->Template("user_recommend_list_ajax"));
	}
