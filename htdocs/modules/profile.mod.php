...
class ModuleObject extends MasterObject
{
	...
	{	
		...
		$act_list = array('search'=>'同城用户','maybe_college'=>'同大学','maybe_friend'=>'同兴趣','usertag'=>'同类人','invite'=>'邀请好友',);
		...
		elseif ('maybe_college' == $act) {
			...
			$per_page_num = 10;
			$query_link = 'index.php?mod=profile&code=maybe_college';
			$where_list = array();		
			...
			if($province){
				...
				if($college){
					$college_name = $this->DatabaseHandler->ResultFirst("select name from ".TABLE_PREFIX."common_college where id = '$college'");
			...
			{ 
				if($province_name){
					...
					if($college_name){
						$where_list['college_name'] = "`college_name`='".addslashes("$college_name")."'";
						$query_link .= "&college=" . $college;
			...
			$hid_college = $college ? $college : $hid_college;
			...
			if($where_list) {
				...
				if($total_record > 0) {
					...
					$uids = array();
					$member_list = $this->TopicLogic->GetMember("{$where} {$order} {$page_arr['limit']}","`uid`,`ucuid`,`username`,`nickname`,`face_url`,`face`,`fans_count`,`topic_count`,`province`,`city`,`college_name`,`aboutme`");
					foreach ($member_list as $_m) {
						$uids[$_m['uid']] = $_m['uid'];		
					}
					...
					if($uids && MEMBER_ID>0) {
						...
						$province = isset($_GET['province']) ? $province : $member['province'];
						$college = isset($_GET['college']) ? $city : $member['college_name'];
	...
	function InviteByEmail()
	{
		...
		foreach($email_list as $email) {
			...
			if (preg_match("~^[-_.[:alnum:]]+@((([[:alnum:]]|[[:alnum:]][[:alnum:]-]*[[:alnum:]])\.)+([a-z]{2,4})|(([0-9][0-9]?|[0-1][0-9][0-9]|[2][0-4][0-9]|[2][5][0-5])\.){3}([0-9][0-9]?|[0-1][0-9][0-9]|[2][0-4][0-9]|[2][5][0-5]))$~i",$email)) {
				...
				$mail_from_email = 'no-reply@pugo.in';
