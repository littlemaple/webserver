...
function pmTopic(i,eid,act)
{
	...
	/*
	if('' == eidVal){
		var myAjax=$.post("ajax.php?mod=pm&code=view_comment",{pmid:i,cod:act},function(d){if(''!=d){$("#"+eid).html(d);}});
	} else {
		$("#"+eid).html('');
	}*/
}
...
function publishSubmit(ci,i,r,topictype,roottid,is_huifu, item, item_id) {
	...
	if( '' == c || '#插入自定义话题#' == c)
    {
		show_message('请输入分享内容',1);
		return false;
	}
	if(c.length < 2)
	{
		show_message('分享内容至少2个字',1);
		return false;
	}
...
function modifyTopic(i,eid,types)
{
	...
	if(''==eidVal){
		var myAjax=$.post(
		"ajax.php?mod=topic&code=modifytopic",
		{
			tid:i,
			types:types
		},
...
function do_modifyTopic(i,imageids,shoppingurl,content,types, handle_key)
{
	//多图
	...
	var shoppingurl = $('#' + shoppingurl).val();
...
/*
  var eidVal = $("#"+"global_select_"+userid).html();
	...
	if(''==eidVal){
		var myAjax=$.post(
		"ajax.php?mod=topic&code=group_menu",
		{
			to_user:userid
		},
		function(d)
...
function topicCommentHTML(tid,tReplys, not_allow_forward,options)
{
	...
	if (!isUndefined(not_allow_forward) && not_allow_forward) {
		...
		type_html = '<input name="topicReplyType_' + tid + '" type="checkbox" id="topicReplyType_' + tid + '" value="reply" onclick="select_checked(\'topicReplyType_' + tid + '\');"/> <label for="topicReplyType_' + tid + '" style="cursor:pointer;">同时转发分享</label>';
	}
	...
	var publishHtml = '<div class="comment_p_t"> <div class="menuf"> <div class="menuf_bq"><b class="reply_ajax_menu_bqb_c"><a href="javascript:viod(0);" onclick="topic_face(\'reply_face_' + tid + '\',\'reply_content_' + tid + '\');return false;">表情</a></b> <div class="forward_f2" style="margin-top:5px;" id="reply_face_' + tid + '" style="border:1px solid #BFBFBF;display:none;"></div> </div></div> <textarea id="reply_content_' + tid + '" name="textarea" class="sp_4 sp_6 operaLay" onkeyup="javas' + 'cript:checkWord(' + thisTopicLength + ',event,\'wordCheckNum_' + tid + '\')" ></textarea> <input type="hidden" id="replytid_' + tid + '" name="replytid_' + tid + '" value="' + tid + '"/> <input type="hidden" id="is_huifu_' + tid + '" name="is_huifu_' + tid + '" value=""/> <input id="rcbtn_' + tid + '" onclick="publishSubmit(\'reply_content_' + tid + '\',\'replytid_' + tid + '\',\'vc\',\'topicReplyType_' + tid + '\',' + tid + ',\'is_huifu_' + tid + '\',\'' + appitem + '\',\'' + appitem_id + '\');$(\'#wordCheckNum_' + tid + '\').html('+thisTopicLength+');" type="button" class="sBtn_2 sBtn_2t" value="评 论" title="按Ctrl+Enter直接发布"/> </div> <div class="comment_p_b"> <div class="comment_p_t2"> '+type_html+' </div> <div class="comment_p_t3"> <span>还可以输入</span> <span><span id="wordCheckNum_' + tid + '" style="color:#FF0000;font-size:12px;">' + thisTopicLength + '</span></span> <span style="width:14px;">字</span> </div> </div><div id="reply_image_uploadify_'  + tid + '" class="comment_p_b2"></div>  <scr' + 'ipt language="Javasc' + 'ript"> $(document).ready(function(){ imageUploadifyHTML(' + tid + ', "reply_content_' + tid + '", "reply_image_uploadify_' + tid + '"); $(".reply_ajax_menu_bqb_c").click(function(){$("#reply_face_' + tid + '").show();}); $(".menu_bqb_c1").click(function(){$("#reply_face_' + tid + '").hide();}); $("#reply_content_' + tid + '").bind("keyup keypress",function(e){ if(e.which==13){return false;} var width=$(this).width(); if (!$.browser.msie )$(this).css("letter-spacing","0.05em"); var len=$(this).val().replace(/[^\\x00-\\xff]/g,"aa").length-1; $(this).height(17*(parseInt(len*6.5/width)+1)); }) }); $("#reply_content_' + tid + '").bind("keydown",function(event){ event = event || window.event; if(event.keyCode == 13 && event.ctrlKey) { $("#rcbtn_' + tid + '").click(); } }); var tReplys = ' + tReplys + ';if(tReplys > 0){topic_view_comment_list(' + tid + ');}initAiInput("reply_content_' + tid + '");</scr' + 'ipt> </p>';
	
    var tcHTML = '<div class="blogTxt b15" id="1234567"> <div class="top_2"></div> <div class="mid"><div id="to_reply_user_' + tid + '"></div>'+publishHtml+'<p class="comment_p h10px"><div id="topic_view_comment_area_' + tid + '"><div id="topic_view_comment_msg_area_' + tid + '"></div></div> </div><div class="bottom"></div> </div>';
...
function topic_view_comment_list(tid)
{
    ...
    var myAjax=$.post("ajax.php?mod=topic&code=view_comment",{tid:tid},function(d){$("#" + eid).html(d);});
}
...
function get_forward_choose(tid, options)
{
	...
	if (!options.noReply) {
		...
		strForwarAndReply = '<input name="topicForwardType_' + tid + '" type="checkbox" id="topicForwardType_' + tid + '" value="forward" onclick="onForwardSelectedListener(' + tid + ',' + strOptions + ');"/> <label for="topicForwardType_' + tid + '" style="cursor:pointer;">转发到分享</label>';
	}
	
 	var handle_key = "forward_menu_"+tid;
    var fcHTML = '<div id="forward_list" class="dialog_inner" style="width:400px;_overflow:hidden;"><div class="mWarp"> <form action="ajax.php?mod=topic&code=forward" method="POST" target="Forwardframe_' + tid + '"><table width="380"><tr><td colspan="2"><div id="toqunwp_' + tid + '" style="display:none;"><div style="float:left">选择微群：</div><div style="float:left" id="qun_select_wp_' + tid + '"></div></div></td></tr><tr> <td colspan="2"><span id="user_new"></span></td> </tr> <tr> <td colspan="2"><span id="topic_forward_content_area_' + tid + '">原文正在加载中……</span></td> </tr> <tr> <td colspan="2">&nbsp;</td> </tr> <tr> <td colspan="2"><div class="alt_3"> <div class="fbqCount" style=" position:absolute; margin:-10px 0 0 285px;*margin:-22px 0 0 -60px;"><style>ul.mycon li{ width:65px;}</style> <ul class="mycon fontGreen" style="width:130px"> <li>还可以输入</li> <li id="wordc"><span id="wordcNum_' + tid + '" style="color:#ff0000;">' + thisTopicLength + '</span></li> <li style="width:14px;">字</li> </ul> </div> <div class="menuf"> <div class="menuf_bq" ><b class="menu_bqb_c"><a href="javascript:viod(0);" onclick="topic_face(\'forward_' + tid + '\',\'forward_content_' + tid + '\');return false;">表情</a></b> <div class="forward_f" id="forward_' + tid + '"></div> </div></div> </div> <div class="alt_3"> <textarea id="forward_content_' + tid + '" name="content" onkeyup="javascr' + 'ipt:checkWord(' + thisTopicLength + ',event,\'wordcNum_' + tid + '\')" class="textarea" onblur="if(this.value == \'\'){this.value = \'转发分享\'; }" onfocus="if(this.value == \'转发分享\'){this.value =\'\'; }" style="width:380px;">转发分享</textarea> </div></td> </tr> <tr> <td width="150"><div class="alt_5">'+strForwarAndReply+'</div> </td> <td align="right"><div class="rb_a1"> <input id="rcbtna_' + tid + '" onclick="Forward(\'forward_content_' + tid + '\',' + tid + ',' + tid + ',\'topicForwardType_' + tid + '\','+strOptions+');return false;" type="button" value="转 发" class="rb_c2"/> <input type="button" name="del" value="取 消" onclick="closeDialog(\'' + handle_key + '\');" class="rb_a2"/> <input name="forward_tid" type="hidden" id="forward_tid" value="{$forward_tid}" /></div> </td> </tr>  <tr> <td colspan="2"> <div id="forward_image_uploadify_' + tid + '"> </div> </td> </tr> </table> </form> </div></div> <sc' + 'ript type="text/jav' + 'ascri' + 'pt"> $(document).ready(function(){ imageUploadifyHTML(' + tid + ', "forward_content_' + tid + '", "forward_image_uploadify_' + tid + '"); $(".menu_bqb_c").click(function(){$("#forward_' + tid + '").show();}); $(".menu_bqb_c1").click(function(){$("#forward_' + tid + '").hide();}); initAiInput("forward_content_' + tid + '");}); $("#forward_content_' + tid + '").bind("keydown",function(event){ event = event || window.event; if(event.keyCode == 13 && event.ctrlKey) { $("#rcbtna_' + tid + '").click(); } }); $("#topic_forward_content_area_' + tid + '").html("<span class=\'loading\'>内容正在加载中，请稍候……</span>");var myAjax=$.post("ajax.php?mod=topic&code=forward_menu",{tid:' + tid + '},function(d){$("#topic_forward_content_area_' + tid + '").html(d);});</scri' + 'pt>';
	
	var fcFrameHtml = '';
	if (!isQunClosed) {
		fcFrameHtml = '<div class="zfbox"><div class="zfTitle" id="zfTitle_'+tid+'"><ul class="zfti"><li id="zf_m1" class="zfhover" onclick="zfTab(\'zf_m\',1,2,' + tid + ')">转发到分享</li><li id="zf_m2" onclick="zfTab(\'zf_m\',2,2,' + tid + ')">转发到微群</li><sub class="menu_zf_c1" onclick="closeDialog(\''+handle_key+'\');"></sub></ul></div><div id="zfcon_zf_m_1" class="zfcon">'+fcHTML+'</div><div id="zfcon_zf_m_2" class="zfcon" style="display:none">转发到微群</div></div>';
	} else {
		fcFrameHtml = '<div class="zfbox"><div class="zfTitle" id="zfTitle_'+tid+'"><ul class="zfti"><li id="zf_m1" class="zfhover">转发到分享</li><sub class="menu_zf_c1" onclick="closeDialog(\''+handle_key+'\');"></sub></ul></div><div id="zfcon_zf_m_1" class="zfcon">'+fcHTML+'</div></div>';
	}
    
    //$("#"+"forward_menu_"+tid).css('display','block');
    //$("#"+"forward_menu_"+tid).html(fcHTML);
	var h = showDialog(handle_key, 'local', '分享给关注我的人', {"html":fcFrameHtml, "noTitleBar":true}, 440);
	//标题栏拖动绑定
...
{	
	...
	MessageBox('confirm', "你确定要删除吗？", '提示', options);
}
 ...
 * 显示主发布对话框
 */
function showMainPublishBox(type)
{	
	...
	if ($('#i_already').length > 0) {
		...
		showDialog(handle_key, 'ajax', '随时随处分享', {url:"ajax.php?mod=misc&code=publishbox&type="+type+"&random="+Math.random()}, 590);
	}
}
...
function view_longtext(idval, tidval, aobj, TPT_id)
{
...
{	
    ...
    (
		...
		function (d) 
        {
            ...
            if(d) {
            	//alert('*.[ectype=\"user_signature_ajax_' + uid + '\"]');
            	 $('*.[ectype=\"user_signature_ajax_' + uid + '\"]').html("(" + d + ")"); 
            	 $('*.[ectype=\"user_signature_ajax_left_' + uid + '\"]').html("(" + d + ")"); 
            }
            else{
      	
            	 var type_html = '<a href="javascript:void(0);" onclick="follower_choose(\'' + uid + '\',\'' + uid + '\',\'topic_signature\'); return false;">编辑个人签名</a> ';            		
            	 
            	 $('*.[ectype=\"user_signature_ajax_' + uid + '\"]').html(""); 
            	 $('*.[ectype=\"user_signature_ajax_left_' + uid + '\"]').html(type_html); 
            }
 ...
 * uid   修改三栏，两栏切换的用户UID
 * web_style  我(当前登录的用户) 选择的分栏状态样式
 * get_code  当前页面 URL地址的 code  在ajax显示页面 起判断显示模块作用
 * ajax_list  显示调用那个ajax模板来作为显示数据模板
 * list_uid  返回用户信息时 显示的UID
 * 
 */
...
{	
	 ...
	 var user_three_tol = $("#"+web_style).val();
	 
	 
	 var style_three_tol = 'undefined' == typeof(user_three_tol) ? web_style : user_three_tol;
 ...
 * 右边栏  ajax 显示用户相关数据
 * @param uid
 * @param showCode   ajax中 code显示的模块
 * @return
*/
function right_show_ajax(uid,showCode,ajax_list)
{
	var ajax_url = '';
	if (showCode == 'my_qun') {
...
function showFollowGroupAddDialog()
{
	...
	setDialogOnCloseListener(handle_key, function(){
		if (__AddFansGroupSuccess__ == true) {
			location.reload();
						...
						if (r != '') {
							...
							$('#topic_simple_share_btn'+options.tid).click(
								function () {
									...
									//publishSimpleTopic($('#topic_simple_content'+options.tid).val(), '', 0, {response:response});
								}
							);
						}
					}
 ...
 * @return
*/
function recommend_user_show_ajax()
{
	var ajax_url = '';
	ajax_url = "ajax.php?mod=user&code=randomUser";
...
//顶 喜欢
function dig(tid,eid,showMsg)
{
	//var events = ('undefined' == typeof(events) ? '' : events);

	var myAjax=$.post(
    	"ajax.php?mod=topic&code=dig",
    	{
    		tid:tid
    	},
    	function(d)
