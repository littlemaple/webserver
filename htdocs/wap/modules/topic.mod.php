		...
		function DoAdd()
	{
	 	 ...
	 	 function DoModify()
	 {
	 	 ...
	 	 if($this->Get['tid'] > 0)
	 	 {
				...
				$query = $this->DatabaseHandler->Query($sql);
				$topicinfo=$query->GetRow();
			
							  $topiclist['content'] = preg_replace('~<U ([0-9a-zA-Z]+)>(.+?)</U>~','',$topicinfo['content']);
	 	 ...
	 	 else
	 	 {
			 ...
			 $topiclist=$query->GetRow();
		 	 ...
		 	 if(empty($content))
		 	 { 
		 	 	 $return_messager = "<span>微博内容不能为空</span>";
	     ...
	     {
	        $this->Messager('编辑成功','index.php?mod=topic&amp;code='.$tid);
	     }
	     else
	     {
